...
function RupiahInput({ value, onChange, error, label, placeholder = "0" }: {
  value: number
    ...
    <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-3 bg-white dark:bg-gray-900">
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs text-gray-500 dark:text-gray-400">
          {t("createTransaction.detail.recipient")} #{detailIndex + 1}
        </span>
        <button
          type="button"
          onClick={onRemove}
        ...
        >
          {t("createTransaction.detail.remove")}
        </button>
      </div>
      <div className="grid grid-cols-3 gap-3 mb-3">
        <TextInput
          label={t("createTransaction.detail.requesterName")}
          required
          placeholder={t("createTransaction.detail.requesterNamePlaceholder")}
        ...
        />
        <TextInput
          label={t("createTransaction.detail.quantity")}
          type="number"
          required
          min={1}
          placeholder="1"
      ...
      <TextInput
        label={t("createTransaction.detail.notes")}
        placeholder={t("createTransaction.detail.notesPlaceholder")}
...
function ItemCard({ itemIndex, control, register, errors, onRemove, watch, setValue }: {
  itemIndex: number
  control: any
  register: any
  errors: any
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-gray-800 dark:text-gray-200">
            {watch(`items.${itemIndex}.item_name`) || t("createTransaction.item.fallbackName", { index: itemIndex + 1 })}
          </span>
        </div>
        <button
          type="button"
        ...
        >
          {t("createTransaction.item.removeItem")}
        </button>
      </div>
      ...
      <div className="p-4 space-y-4 bg-gray-50/30 dark:bg-gray-900/30">
        <div className="grid grid-cols-3 gap-3">
          <div className="col-span-2">
            <TextInput
              label={t("createTransaction.item.itemName")}
              required
              placeholder={t("createTransaction.item.itemNamePlaceholder")}
          ...
          {listCategory && (
            <Controller
              ...
              render={({ field, fieldState }) => (
                <Selects
                  label={t("createTransaction.item.category")}
                  value={field.value ?? ""}
                  onChange={(v) => field.onChange(Number(v))}
                  options={activeCategoryListToSelectOptions(listCategory.data)}
                  placeholder={t("createTransaction.item.categoryPlaceholder")}
                  error={fieldState.error?.message}
                  labelClassName="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
                  selectClassName="text-sm py-2"
                  required
        ...
        <div className="grid grid-cols-3 gap-3">
          <TextInput
            label={t("createTransaction.item.quantity")}
            type="number"
            required
            min={1}
            placeholder="1"
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <RupiahInput
                label={t("createTransaction.item.unitPrice")}
                value={field.value}
                onChange={field.onChange}
                error={fieldState.error?.message}
              />
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            {t("createTransaction.item.itemNotes")}
          </label>
          <input
            type="text"
            placeholder={t("createTransaction.item.itemNotesPlaceholder")}
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {t("createTransaction.item.recipientDetail")}
              ...
              <span className="text-xs text-gray-400">({t("createTransaction.item.optional")})</span>
            </div>
          ...
          {isQtyExceeded && (
            <div className="mb-3 flex items-center gap-2 text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2">
              {t("createTransaction.item.qtyExceededWarning", { total: totalDetailQty, max: itemQty })}
            </div>
          )}
...
export default function CreateTransactionPage() {
  ...
  const totalNilai = watchedItems.reduce((sum, item) => sum + (Number(item?.quantity) || 0) * (Number(item?.unit_price) || 0), 0)
    ...
    <section className="space-y-5 mt-4">
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-200">{t("createTransaction.section.transactionInfo")}</h3>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            {t("createTransaction.section.transactionNotes")}
          </label>
          <textarea
            rows={2}
            placeholder={t("createTransaction.section.transactionNotesPlaceholder")}
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-200">{t("createTransaction.section.itemList")}</h3>
            <span className="text-xs bg-gray-100 dark:bg-gray-800 text-gray-500 px-1.5 py-0.5 rounded-full">
              {t("createTransaction.section.itemCount", { count: itemFields.length })}
            </span>
          </div>
        </div>
        ...
        >
          <span className="text-base leading-none">+</span> {t("createTransaction.section.addItem")}
        </button>
      </div>
      ...
      <div className="flex items-center justify-between bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl px-5 py-4">
        <div className="flex gap-8">
          <div>
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-0.5">{t("createTransaction.summary.totalItems")}</p>
            <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">
              {t("createTransaction.summary.itemsValue", { types: itemFields.length, units: totalUnit })}
            </p>
          </div>
          <div>
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-0.5">{t("createTransaction.summary.totalValue")}</p>
            <p className="text-sm font-semibold text-gray-800 dark:text-gray-200">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {t("createTransaction.summary.saveDraft")}
          </button>
          <button
          ...
          >
            {isPending ? t("createTransaction.summary.submitting") : t("createTransaction.summary.submit")}
          </button>
        </div>
      </div>
    </section>
