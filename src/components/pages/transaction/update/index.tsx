// pages/transaction/edit/index.tsx
...
import { useUpdateProcurement } from "../../../../hooks/mutation/transaction/update"
...
// ─── Reusable Inputs (sama persis dengan Create) ────────────────────────────
...
function RupiahInput({
    ...
    // sync display kalau value dari luar berubah (saat data di-load)
    useEffect(() => {
        setDisplay(value ? toRupiah(value) : "")
    }, [value])
...
function ItemCard({
    ...
    // auto-expand kalau ada details
    useEffect(() => {
        if (detailFields.length > 0) setShowDetails(true)
    }, [detailFields.length])
        ...
        <section className="space-y-5 mt-4">
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-5 space-y-4">
                ...
                <div className="border border-gray-200 dark:border-gray-700 rounded-xl p-4 space-y-3">
                    <div className="grid grid-cols-3 gap-3">
                        <SkeletonBlock className="col-span-2 h-9" />
...
export default function EditTransactionPage() {
    const { "*": id } = useParams()
    const { data: detailData, isLoading } = useTransactionDetail(id || "")
    ...
    // isi form setelah data dari API ready
    useEffect(() => {
        if (!detailData?.data) return

        const { transaction, items } = detailData.data
        ...
        <section className="space-y-5 mt-4">
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-5">
                <div className="flex items-center gap-2 mb-4">
                    ...
                    {/* badge nomor transaksi (read-only info) */}
                    {detailData?.data.transaction.transaction_number && (
                        <span className="ml-auto text-xs font-mono text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded-md">
                            {detailData.data.transaction.transaction_number}
                        </span>
                    )}
                </div>
