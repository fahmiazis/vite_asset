...
export default function DetailTransactionLayout({ data }: { data: detailTransactionWStageProps }) {
  ...
  const pendingApproval = approvals.find(
    (a: approvalStatusState) => a.status?.toLowerCase() === 'pending'
  )
  ...
  return (
    <section className="space-y-4 mt-4">
      ...
      <div className="bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-700 rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-200">Alur Approval</h3>
          {totalSteps > 0 && (
            <span className="text-xs text-gray-500 dark:text-gray-400">
              {completedSteps}/{totalSteps} step selesai
            </span>
          )}
        </div>
        ...
        {isLoadingApproval ? (
          ...
          <p className="text-xs text-gray-400 text-center py-4">Belum ada data approval</p>
        ) : (
          <div>
                  ...
                  <div className={`pb-4 flex-1`}>
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <p className="text-xs font-semibold text-gray-800 dark:text-gray-100">
                          {approval.flow_step?.step_name ?? `Step ${index + 1}`}
                        </p>
                    ...
                    {isApproved && approval.updated_at && (
                      <div className="mt-1.5 flex items-center gap-1 text-xs text-emerald-600 dark:text-emerald-400">
                        ...
                        Disetujui pada {new Date(approval.updated_at).toLocaleString("id-ID", {
                          day: "2-digit", month: "short", year: "numeric",
                          hour: "2-digit", minute: "2-digit",
                        })}
                        {approval.approved_by && (
                          <span className="text-gray-400 dark:text-gray-500 ml-1">· oleh {approval.approved_by}</span>
                        )}
                      </div>
                    )}
                    ...
                    {isRejected && approval.rejected_at && (
                      <div className="mt-1.5 flex items-center gap-1 text-xs text-red-500 dark:text-red-400">
                        ...
                        Ditolak pada {new Date(approval.rejected_at).toLocaleString("id-ID", {
                          day: "2-digit", month: "short", year: "numeric",
                          hour: "2-digit", minute: "2-digit",
                        })}
                        {approval.rejected_by && (
                          <span className="text-gray-400 dark:text-gray-500 ml-1">· oleh {approval.rejected_by}</span>
                        )}
                      </div>
                    )}
